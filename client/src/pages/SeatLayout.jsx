...
const SeatLayout = () => {
    ...
    const handleSeatClick = (seatId) => {
        ...
        if (!selectedSeats.includes(seatId) && selectedSeats.length > 9) {
            return toast('You can only select a maximum of 10 seats.')
        }
    ...
    const getOccupiedSeats = async () => {
        ...
        console.log("Fetching for Show ID:", selectedTime.showId); // ✅ Add this

        try {
            const { data } = await axios.get(`/api/booking/seats/${selectedTime.showId}`);
            if (data.success) {
                console.log("Occupied Seats:", data.occupiedSeats); // ✅ Confirm what backend returns
    ...
    const bookTickets = async () => {
        try {
            ...
            if (!selectedTime || !selectedSeats.length) {
                return toast.error('Please select a timing and select at least one seat.');
            }
    ...
    return show ? (
        <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
            ...
            <div className='relative flex flex-1 flex-col items-center max-md:mt-16'>
                ...
                <p className='text-sm text-gray-400 mb-6'>This side Screen</p>
